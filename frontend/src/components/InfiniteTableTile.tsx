...
type InfiniteTableTileProps<T, U> = {
  ...
  apiCall: (
    currPageNum: string,
    queryParams: U
  ) => Promise<AxiosResponse<IInfTablePage<T>, any>>;
  queryParams: U;
  modal: ReactNode;
};

const InfiniteTableTile = <T, U>({
  title,
  columns,
  apiCall,
  queryParams,
  modal,
}: InfiniteTableTileProps<T, U>) => {
  ...
  const renderTopToolbarCustomActions = ({}) => (
    <Group>
      <Title order={2} style={{ padding: "10px" }}>
        {title}
      </Title>
      {modal}
    </Group>
    ...
    <div className={"tile full-tile"}>
      <MantineReactTable
        ...
        mantineToolbarAlertBannerProps={{
          ...
          children: !noFilters
            ? "Deselect filters to load more results"
            : "Error loading data",
        }}
        ...
        renderBottomToolbarCustomActions={() => (
          <Text>Fetched {totalFetched} rows.</Text>
